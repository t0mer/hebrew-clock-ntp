...
// Hebrew word clock — Seeed XIAO 7.5" e-paper driver board (combo 502).
// The sketch's driver.h selects this board, but the sketch folder is NOT on the
// include path when the Seeed_GFX library's own .cpp files are compiled, so
// __has_include("driver.h") below fails for those translation units. That left
// TFT_eSPI.cpp built without EPAPER_ENABLE and produced "undefined reference to
// EPaper::..." at link time. Defining the board here makes every translation
// unit (sketch + library) agree on combo 502. Guarded by #ifndef so a sketch
// that already set it (or a different board) is left untouched.
